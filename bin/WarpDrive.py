...
def load_process(inputfile_path, result_file_path, receptor_path, log, vina_config):
    cmd = "vina --config {0} --ligand {1} --out {2} --log {3} --receptor {4}". \
        format(vina_config, inputfile_path, result_file_path, log, receptor_path)
    subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    return None



def driver(pdbqt_path, results_path, receptor_path, log_path, vina_config, show_queue=False, scaling_factor=1, delay=0):
    counter = 0
    ...
    while counter < len(file_list):
        ...
        # Set log file and path
        if log_path[-1] == "/":
            logfile = "{0}{1}".format(log_path, infile)
        else:
            logfile = "{0}/{1}".format(log_path, infile)
        ...
        if running_processes <= (num_cpus * scaling_factor):
            ...
            load_process(inputfile_path=infile_path, result_file_path=result_file_path, log=logfile,
...
if __name__ == '__main__':
    ...
    args.add_argument('-logs', help="[REQUIRED] Path to the log file output folder", required=True, type=str)
    ...
    start = time.time()
    driver(pdbqt_path=pdbqt_path, results_path=results, log_path=log_path, receptor_path=receptor_path,
